...
//        System.out.println("出版社："+publisher.getText());
    ...
    public static void test() {
        File file = new File("D:\\IdeaProjects\\code-learn\\mybatis\\src\\main\\resources\\mapper\\User.xml");
        SAXReader reader = new SAXReader();
        try {
            Document fileDocument = reader.read(file);
            Element rootElement = fileDocument.getRootElement();
            System.out.println("root=" + rootElement.getName() + " " + rootElement.getText());
            // mapper
            List<Element> childElements = rootElement.elements();//获取当前元素下的全部子元素

            int index = 0;
            for (Element child : childElements) {//循环输出全部book的相关信息
                String namespace = childElements.get(0).attributeValue("namespace");

                String parameterType = child.element("select").attributeValue("parameterType");
                String resultType = child.element("select").attributeValue("resultType");
                System.out.println(namespace + "_" + parameterType + "_" + resultType);
                // mapper节点子元素
                List<Element> books = child.elements();
                for (Element book : books) {
                    String name = book.getName();//获取当前元素名
                    String text = book.getText();//获取当前元素值
                    System.out.println(index + ":name= " + name + " text: " + text);
                    index++;
                }
            }
